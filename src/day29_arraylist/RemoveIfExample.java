...
public class RemoveIfExample {
    public static void main(String[] args) {
        ...
        //remove the elements from integer arraylist that is divisible by 2

        list1.removeIf(each-> each %2 == 0);
        System.out.println(list1);

        ArrayList<Integer> list2 = new ArrayList<>(original);
        list2.removeIf(each->each %2 ==1);
        System.out.println(list2);

        ArrayList<Integer> list3 = new ArrayList<>(original);
        list3.removeIf(each->each >100);
